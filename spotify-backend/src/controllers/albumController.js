...
const addAlbum = async (req, res) => {
    try {
        ...
        if (!name?.trim() || !desc?.trim() || !bgColour?.trim()) {
            return res.status(400).json({ success: false, message: "Name, description and background colour are required" });
        }
        if (!imageFile) {
        ...
        res.status(500).json({ success: false, message: "Album Add Failed" })
    }
}

const listAlbum = async (req, res) => {
    try {
        const allAlbums = await Album.find({}).sort({ _id: -1 });
        res.status(200).json({ success: true, albums: allAlbums });
    } catch (error) {
        console.log('Failed at listAlbum, ', error);
        res.status(500).json({ success: false, message: "Album List Failed" })
    }
}
